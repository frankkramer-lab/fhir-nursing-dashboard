...
the ```filterData.js``` file in the ```src/utils``` folder. The new DataProcessor needs to be added to
the ```dataProcessors``` array in the ```filterData.js``` file. You can orient on the existing DataProcessors in the
file.